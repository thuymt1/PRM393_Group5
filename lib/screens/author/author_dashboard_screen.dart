...
class _AuthorDashboardScreenState extends State<AuthorDashboardScreen> {
  ...
  Widget _buildDashboardTab() {
    ...
    return FutureBuilder<List<dynamic>>(
      future: _apiService.getMyArticles(),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 32),
                _buildSectionHeader('Bài viết gần đây nhất', purpleColor, showSeeAll: true, onSeeAll: () {
                  _onTabTapped(1);
                }),
                const SizedBox(height: 12),
                myArticles.isEmpty
                    ? _buildEmptyState('Bạn chưa viết bài viết nào.')
                    : _buildRecentArticleItem(myArticles.first, purpleColor),
  ...
  Widget _buildStatsGrid(Color accentColor, int articlesCount) {
    return GridView.count(
      ...
      children: [
        _statItem('Bài viết', '$articlesCount', Icons.article_outlined, accentColor),
        _statItem('Lượt đọc', '1.240', Icons.visibility_outlined, accentColor),
        _statItem('Lượt thích', '320', Icons.favorite_border_outlined, accentColor),
  ...
  Widget _buildRecentArticleItem(Map<String, dynamic> article, Color color) {
    ...
    final String dateStr = '${createdAt.day}/${createdAt.month}/${createdAt.year}';
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const Text('1.250', style: TextStyle(fontSize: 10, color: Colors.grey)),
                      const SizedBox(width: 12),
                      const Icon(Icons.favorite_border_outlined, size: 12, color: Colors.grey),
                      const SizedBox(width: 4),
                      const Text('320', style: TextStyle(fontSize: 10, color: Colors.grey)),
                      const Spacer(),
                      Text(
                        dateStr,
            ...
            child: articles.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      return _buildRecentArticleItem(articles[index], color);
                    },
                  ),
          ),
        );
  ...
  Widget _buildProfileTab() {
    ...
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator(color: activeColor));
        }
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        backgroundImage: avatarUrl != null && avatarUrl.isNotEmpty
                            ...
                            : const NetworkImage('https://i.pravatar.cc/150?u=author'),
                      ),
                      const SizedBox(height: 16),
                      Text(
                        fullName,
                      ...
                      Container(
                        ...
                        child: const Row(
                          ...
                          children: [
                            Icon(Icons.verified, color: activeColor, size: 16),
                            SizedBox(width: 8),
                            Text(
                              'Người viết bài',
