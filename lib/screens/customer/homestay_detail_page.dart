...
class HomestayDetailPage extends ConsumerStatefulWidget {
  const HomestayDetailPage({super.key});

  @override
  ConsumerState<HomestayDetailPage> createState() => _HomestayDetailPageState();
}

class _HomestayDetailPageState extends ConsumerState<HomestayDetailPage> {
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      if (originalState) {
        await ref.read(homestayRepositoryProvider).removeFavorite(_homestay.id);
      } else {
        await ref.read(homestayRepositoryProvider).addFavorite(_homestay.id);
      }
    } catch (e) {
      if (mounted) {
        setState(() {
