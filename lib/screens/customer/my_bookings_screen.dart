...
class _MyBookingsScreenState extends State<MyBookingsScreen> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length:
          3, // Định cấu hình số lượng Tab hiển thị (Sắp tới, Đã xong, Đã hủy)
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors
              .white, // Nền trắng giúp phần thanh TabBar hiển thị tách biệt rõ ràng
          ...
          bottom: const TabBar(
            labelColor: Color(
              0xFFE07A5F,
            ), // Sắc cam cam thương hiệu cho nhãn Tab được chọn
            unselectedLabelColor:
                Colors.grey, // Sắc xám cho các nhãn Tab chưa được chọn
            indicatorColor: Color(
              0xFFE07A5F,
            ), // Màu đường gạch chân chỉ báo Tab đang hoạt động
        ...
        body: TabBarView(
          children: [
            _buildBookingList(context, 'pending'),
      ...
      builder: (context, snapshot) {
        ...
        final bookings = (snapshot.data ?? [])
            ...
            .map<Map<String, dynamic>>((b) {
              final home = b['homestays'] as Map<String, dynamic>? ?? {};
              final images = home['homestay_images'] as List? ?? [];
              return {
                ...b as Map<String, dynamic>,
                'name': home['name'] ?? 'Homestay',
                'location': '${home['city'] ?? ''} ${home['address'] ?? ''}',
                'date': '${b['check_in']} - ${b['check_out']}',
  ...
  Widget _buildBookingCard(BuildContext context, Map<String, dynamic> booking) {
    ...
    switch (booking['status']) {
      case 'Đã xác nhận':
        ...
        break;
      case 'Hoàn thành':
        statusColor = const Color(0xFF6D4C41);
        break; // Sắc nâu hệ thống cho đơn đã xong
      case 'Đã hủy':
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(
          24,
        ), // Bo tròn 4 góc khung thẻ 24 đơn vị
        boxShadow: [
          BoxShadow(
            color: Colors.black.withOpacity(
              0.03,
            ), // Đổ bóng mờ mịn siêu nhẹ tạo độ sâu bề mặt tinh tế
            blurRadius: 10,
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.circular(
          24,
        ), // Đảm bảo hiệu ứng sóng nước hiệu ứng gợn sóng (Ink Splash) bo khít góc thẻ
        child: Column(
          children: [
              ...
              child: Stack(
                children: [
                  Image.network(
                    ...
                    fit: BoxFit
                        .cover, // Cắt cúp tỉ lệ ảnh cân đối phủ kín khung ngang
                  ),
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.white.withOpacity(
                          0.9,
                        ), // Khung nền trắng đục mờ trong suốt 90%
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: const TextStyle(
                          ...
                          color: Color(
                            0xFFE07A5F,
                          ), // Sắc cam nổi bật hiển thị chi phí tổng thanh toán
                        ),
                      ),
                    ],
                  ),
                  ...
                  const Divider(
                    height: 24,
                  ), // Đường kẻ vạch phân tách mảnh ngăn cách mốc thời gian lưu trú
                  Row(
                    children: [
                      const Icon(
                      ...
                      const Icon(
                        ...
                        size: 18,
                        color: Color(0xFFE07A5F),
                      ), // Mũi tên hướng đi tới màu cam
                    ],
                  ),
                  if (booking['raw_status'] == 'confirmed') ...[
                    const SizedBox(height: 12),
                    SizedBox(
                      width: double.infinity,
                      child: OutlinedButton.icon(
                        onPressed: booking['can_review'] == true
                            ? () => Navigator.pushNamed(
                                context,
                                '/create-review',
                                arguments: {
  ...
  Widget _buildBottomNavBar() {
    return BottomNavigationBar(
      type: BottomNavigationBarType
          .fixed, // Cố định kiến trúc tránh hiệu ứng nhảy dịch chuyển vị trí khi click chọn
      selectedItemColor: const Color(
        0xFFE07A5F,
      ), // Sắc cam làm nổi bần bật icon tab đang đứng hoạt động tích cực
      unselectedItemColor: Colors
          .grey, // Sắc màu xám nhẹ cho các danh mục tab còn lại chưa được lựa chọn
      currentIndex:
          2, // Đánh chỉ mục vị trí hiện tại đang nằm cố định ở Tab thứ 2 ('Chuyến đi' của tôi)
      items: const [
  ...
  Future<void> _showReviews(BuildContext context, int homestayId) async {
    final reviews = await ApiService().getHomestayReviews(homestayId);
    if (!context.mounted) return;
    showModalBottomSheet(
      context: context,
      builder: (_) => reviews.isEmpty
          ? const SizedBox(
              height: 160,
              child: Center(child: Text('Homestay chưa có feedback')),
            )
          : ListView.builder(
              padding: const EdgeInsets.all(20),
