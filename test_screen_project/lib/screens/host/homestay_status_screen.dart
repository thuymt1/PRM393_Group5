...
class HomestayStatusScreen extends StatefulWidget {
  ...
  const HomestayStatusScreen({super.key, this.homestay});
...
class _HomestayStatusScreenState extends State<HomestayStatusScreen>
  ...
  Widget _buildPropertyPreview() {
    ...
    final imageUrl = (homestay != null && homestay.images.isNotEmpty)
        ? homestay.images[0]
        : 'https://images.unsplash.com/photo-1510798831971-661eb04b3739?q=80&w=1000';
    final name = homestay?.name ?? 'The Terracotta Nest';
    final location = homestay != null
        ? '${homestay.address}, ${homestay.city}'
        : 'Đà Lạt, Lâm Đồng';
    final price = homestay != null
        ? formatCurrency.format(homestay.pricePerNight)
        : '1.250.000đ';
    final rating = homestay?.rating.toStringAsFixed(1) ?? '4.9';
    ...
    return Row(
      children: [
        _insightItem('1.240', 'Lượt xem\ntháng này',
            Icons.visibility_outlined, const Color(0xFF6366F1)),
        const SizedBox(width: 12),
        _insightItem('18', 'Đặt phòng\nhoàn thành',
            Icons.book_online_outlined, const Color(0xFFE07A5F)),
        const SizedBox(width: 12),
        _insightItem(
          ...
          children: [
            Expanded(
              child: _actionButton(
                ...
                () => context.push('/add-homestay-basic-info'),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: _actionButton(
                ...
                () => ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Tính năng quản lý lịch sắp ra mắt'),
              ...
              child: _actionButton(
                ...
                () {
                  if (widget.homestay != null) {
                    context.push('/homestay-detail', extra: widget.homestay);
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Không có dữ liệu homestay để xem thử'),
                        behavior: SnackBarBehavior.floating,
                      ),
                    );
                  }
